...
export function OpenLifecycleQueue() {
  ...
  if (items.length === 0) {
    return (
      <div className="mb-6 rounded-lg border border-dashed border-line bg-sand px-4 py-3">
...
function QueueColumn({
  ...
  return (
    <div
      className={
        borderLeft ? "border-t border-line md:border-l md:border-t-0" : ""
      }
    >
      <p className="border-b border-line px-4 py-2 text-xs font-medium uppercase tracking-wide text-muted">
        {title} ({items.length})
      </p>
      {items.length === 0 ? (
